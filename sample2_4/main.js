...
function _update() {
  ...
  ball.forEach((val,i) => {
    if(val.cnt++ % 60 == 0) {
      ...
      // 背景も
      if(i == 0) document.body.style.backgroundColor = _getRandomColor();
    }

    // 背面と前面ので変える
    if(i == 0) {
      // 背面
      val.size = window.innerWidth * 0.75;
      val.x = centerX + Math.sin(radian(ang * 0.28)) * radius;
      val.y = centerY + Math.cos(radian(ang * 0.28)) * radius;
    } else {
      // 前面
      val.size = window.innerWidth * 0.25;
      val.x = centerX + Math.sin(radian(ang * 1)) * radius * 0.5;
      val.y = centerY + Math.cos(radian(ang * 1)) * radius * 0.5;
    }
